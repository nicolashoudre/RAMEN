...
class AI4SmallFarms(RawGeoFMDataset):
    ...
    def __getitem__(self, index):
        ...
        # Resize to (496, 496) if needed
        if image.shape[-2:] != (496, 496):
            image = F.resize(image, [496, 496])
        if target.shape[-2:] != (496, 496):
            # Use nearest interpolation for masks to avoid altering class indices
            target = F.resize(target.unsqueeze(0).float(), [496, 496], interpolation=F.InterpolationMode.NEAREST).squeeze(0).long()
