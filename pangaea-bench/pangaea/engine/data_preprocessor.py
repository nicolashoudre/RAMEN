        ...
        for key, value in data.items():
            if isinstance(value, dict):
                ...
                for subkey, subvalue in value.items():
                    if subkey.endswith("_dates"):
                        saved_dates[key][subkey] = subvalue
                # remove them from data before preprocessing
                for subkey in saved_dates[key]:
                    del value[subkey]
    
        for process in self.preprocessor:
            data = process(data)
    
        for key, subdict in saved_dates.items():
