...
class MetricsMonoModal(Metric):
    ...
    def compute(self):
        if self.get_classes:
            f1 = self.f1.compute()
            out = {'OA': self.acc.compute(), 'F1_Score_macro': sum(f1)/len(f1), 'F1_Score_micro': self.f1_micro.compute(), 'F1_Score_weighted': self.f1_weighted.compute()}
...
class MetricsSemSeg(Metric):
    ...
    def update(self, pred, gt):
        label = gt['label'].flatten(1,2).flatten(0, 1).long()
