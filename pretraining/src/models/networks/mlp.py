...
from models.networks.encoder.utils.ramen_utils import Resampler
...
class MLPResampleSemSeg(nn.Module):
    def __init__(
            self,
            initial_dim=512,
            hidden_dim=[128, 32, 2],
        ...
        args = self.init_layers(dim, norm, activation)
        self.mlp = nn.Sequential(*args)
        self.resampler = Resampler(initial_dim)
    ...
    def forward(self, x):
        ...
        if self.num_iterations > 0:
            scale = (self.res / self.mask_res) ** (1 / self.num_iterations)
            for i in range(self.num_iterations):
                x = self.resampler(x, scale=scale)
        else:
            scale = (self.res / self.mask_res)
